-- ============================================================
-- Migration 023 — Corrige "column reference \"id\" is ambiguous"
-- no login.
--
-- Causa: login_player() declara uma coluna de saída chamada `id`
-- (via RETURNS TABLE), o que cria uma variável PL/pgSQL `id` no
-- escopo da função. O UPDATE final (`where id = v_player.id`) ficava
-- ambíguo entre essa variável e a coluna public.players.id.
--
-- Correção: usa um apelido de tabela explícito (p) no UPDATE.
